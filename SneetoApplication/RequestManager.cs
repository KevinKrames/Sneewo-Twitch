...
namespace SneetoApplication
{
    public class RequestManager
    {
        ...
        public List<Episode> availableEpisodes;
        ...
        public RequestManager()
        {
            ...
            availableEpisodes = new List<Episode>();
        }
        ...
        public void Update()
        {
            ...
            if (!Clearing && waitingForChatGPT == false && waitingForValidation == false && waitingForTTSGPT == false && requestsOutgoing.Count == 0)
            {
                if (availableEpisodes.Count == 0)
                {
                    string[] dirs = Directory.GetDirectories($"{Brain.configuration["oneDrive"]}processed", "*", SearchOption.TopDirectoryOnly);
                    
                    foreach (var dir in dirs)
                    {
                        availableEpisodes.Add(new Episode { fileLocation = dir, directoryName = dir.Substring(dir.LastIndexOf("\\") + 1) });
                    }
                    availableEpisodes.Shuffle();
                }
                var nextEpisode = availableEpisodes[0];
                if (nextEpisode != null && ChannelManager.Instance.Channels != null && ChannelManager.Instance.Channels.Count > 0)
                {
                    var channel = ChannelManager.Instance.Channels?.Values?.ToList()[0];
                    var sitcomRequest = new SitcomRequest { User = "Autogenerated", File = nextEpisode.directoryName, Validated = true, RequestState = RequestState.WaitingForVoices, channel = channel };
                    var message = new TTSPythonMessage { inputText = $"{nextEpisode.directoryName}.txt" };
                    TTSPython.inputToPython.Enqueue(message);
                    sitcomRequest.RequestState = RequestState.WaitingForVoices;
                    waitingForTTSGPT = true;
                    requestsOutgoing.Add(sitcomRequest);
                    availableEpisodes.Remove(nextEpisode);
                }
            }
